...
const Discussion = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(`https://kisanseva-backend.onrender.com/api/v1/user/${userId}`);
  ...
  const fetchThreads = async (option) => {
    ...
    try {
      let url = '';
      if (option === 'All') {
        url = 'https://kisanseva-backend.onrender.com/api/v1/tweets';
      } else if (option === 'Latest') {
        url = 'https://kisanseva-backend.onrender.com/api/v1/tweets';
      } else if (option === 'Popular') {
        url = 'https://kisanseva-backend.onrender.com/api/v1/threads/popular';
      } else {
        url = `https://kisanseva-backend.onrender.com/api/v1/threads/mytweets/${userId}`;
      }
